"""
Main training script for CNN-based chess evaluation model
Uses convolutional layers to learn spatial patterns from board representation.
...
try:
    ...
    from train.cnn_model.model import ChessCNNModel, board_to_tensor, count_parameters
...
class ChessBoardDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            Tuple of (board_tensor, evaluation)
        """
        ...
        # Convert board to tensor (board_to_tensor handles turn-based flipping)
        board_tensor = board_to_tensor(board)

        # If it's black to move, flip the evaluation sign
        ...
        return board_tensor.squeeze(0), eval_tensor  # Remove batch dimension
...
def collate_fn(batch: list) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    board_tensors = torch.stack([item[0] for item in batch])
    evaluations = torch.stack([item[1] for item in batch])

    return board_tensors, evaluations
...
def create_dataloaders(train_path: str, val_path: Optional[str] = None, 
    ...
    # Adjust prefetch factor based on device - MPS has less memory than CUDA
    if device is not None and device.type == 'mps':
        prefetch_factor = 4 if num_workers > 0 else None  # Lower for MPS to reduce memory pressure
    else:
    ...
    if val_path:
        ...
        val_loader = DataLoader(
            ...
            collate_fn=collate_fn,
            pin_memory=use_pin_memory,  # Only use with CUDA
            prefetch_factor=prefetch_factor,  # Use same prefetch factor as training loader
...
def train_epoch(model: torch.nn.Module, train_loader: torch.utils.data.DataLoader,
    ...
    for batch_idx, (board_tensors, targets) in enumerate(pbar):
        # Move to device (non_blocking only works for CUDA, not MPS)
        if device.type == 'cuda':
...
def validate(model: torch.nn.Module, val_loader: torch.utils.data.DataLoader,
    ...
    with torch.no_grad():
        ...
        for board_tensors, targets in pbar:
            # Move to device
            board_tensors = board_tensors.to(device)
            targets = targets.to(device)
    ...
    if upload_to_hf and config.hf_auto_upload:
        def upload_in_background():
            try:
                ...
                model_config = {
                    'input_channels': 12,
                    'conv_channels': getattr(config, 'conv_channels', 64),
                    'num_residual_blocks': getattr(config, 'num_residual_blocks', 4),
                    'dense_hidden': getattr(config, 'dense_hidden', 256),
...
def train(config: TrainingConfig) -> Tuple[torch.nn.Module, Dict[str, list]]:
    ...
    print("Using CNN-based model architecture")
