...
from ..dataset import (
    ChessPositionDatasetSparse,
    collate_sparse,
    create_dataloaders_sparse,
)
from ..download_data import download_and_process_lichess_data
from ..upload_to_hf import upload_model_to_hf
from .model import BitboardFeatures, ChessNNUEModel, count_parameters
    ...
    def __init__(
        ...
        if self.normalize and len(self.evaluations) > 0:
            ...
            if self.eval_std < 1e-6:  # Avoid division by zero
                self.eval_std = 1.0
            print(
                f"Loaded {len(self.positions)} positions from {data_path} (bitmap mode)"
            )
            print(
                f"Evaluation stats: mean={self.eval_mean:.2f}, std={self.eval_std:.2f}"
            )
        else:
            print(
                f"Loaded {len(self.positions)} positions from {data_path} (bitmap mode)"
...
def collate_fn_bitmap(batch: list) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    evaluations = torch.tensor(evaluations, dtype=torch.float32).unsqueeze(
        1
    )  # [batch_size, 1]
...
def create_dataloaders(
    ...
    train_dataset = ChessBoardDatasetBitmap(
        train_path,
    ...
    if val_path:
        ...
        val_dataset = ChessBoardDatasetBitmap(
            val_path,
            max_positions=max_val_positions,
            score_min=score_min,
            score_max=score_max,
            normalize=False,
        )  # Don't recompute stats
...
def get_loss_function(
    config: TrainingConfig, eval_std: Optional[float]
) -> torch.nn.Module:
...
def validate(
    ...
    if eval_std is not None and eval_std > 0:
        ...
        metrics["accuracy_100cp"] = (
            errors_centipawns <= 100
        ).float().mean().item() * 100
        metrics["accuracy_200cp"] = (
            errors_centipawns <= 200
        ).float().mean().item() * 100
...
def train(config: TrainingConfig) -> Tuple[torch.nn.Module, Dict[str, list]]:
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        training_history["train_rmse_normalized"].append(
            train_metrics["rmse_normalized"]
        )
        training_history["train_mae_normalized"].append(train_metrics["mae_normalized"])
        if "rmse_centipawns" in train_metrics:
            training_history["train_rmse_centipawns"].append(
                train_metrics["rmse_centipawns"]
            )
            training_history["train_mae_centipawns"].append(
                train_metrics["mae_centipawns"]
            )
        ...
        print(
            f"Epoch {epoch+1}/{config.num_epochs} - Train {train_loss_str}, LR: {current_lr:.6f}{warmup_status}"
        )
        ...
        if has_validation and (epoch + 1) % config.validate_every_n_epochs == 0:
            val_metrics = validate(
                model, val_loader, criterion, device, config, eval_std
            )
            val_loss = val_metrics["loss"]
            training_history["val_loss"].append(val_loss)
            training_history["val_rmse_normalized"].append(
                val_metrics["rmse_normalized"]
            )
            training_history["val_mae_normalized"].append(val_metrics["mae_normalized"])
            if "rmse_centipawns" in val_metrics:
                training_history["val_rmse_centipawns"].append(
                    val_metrics["rmse_centipawns"]
                )
                training_history["val_mae_centipawns"].append(
                    val_metrics["mae_centipawns"]
                )
                training_history["val_accuracy_50cp"].append(
                    val_metrics.get("accuracy_50cp", 0)
                )
                training_history["val_accuracy_100cp"].append(
                    val_metrics.get("accuracy_100cp", 0)
                )
                training_history["val_accuracy_200cp"].append(
                    val_metrics.get("accuracy_200cp", 0)
                )
                training_history["val_median_error_centipawns"].append(
                    val_metrics.get("median_error_centipawns", 0)
                )
                training_history["val_p95_error_centipawns"].append(
                    val_metrics.get("p95_error_centipawns", 0)
                )
            ...
            if val_loss_smooth is None:
                ...
                val_loss_smooth = (
                    val_loss_smooth_alpha * val_loss_smooth
                    + (1 - val_loss_smooth_alpha) * val_loss
                )
        ...
        if save_regular and (not config.save_best_only or not has_validation):
            save_checkpoint(
                ...
                upload_to_hf=config.hf_auto_upload,
                upload_threads=upload_threads,
                scheduler=scheduler,
                best_val_loss=best_val_loss,
                training_start_time=training_start_time,
    ...
    if last_train_loss is None:
        ...
        if has_validation and (config.num_epochs % config.validate_every_n_epochs != 0):
            ...
            training_history["val_rmse_normalized"].append(
                final_val_metrics["rmse_normalized"]
            )
            training_history["val_mae_normalized"].append(
                final_val_metrics["mae_normalized"]
            )
            if "rmse_centipawns" in final_val_metrics:
                training_history["val_rmse_centipawns"].append(
                    final_val_metrics["rmse_centipawns"]
                )
                training_history["val_mae_centipawns"].append(
                    final_val_metrics["mae_centipawns"]
                )
                training_history["val_accuracy_50cp"].append(
                    final_val_metrics.get("accuracy_50cp", 0)
                )
                training_history["val_accuracy_100cp"].append(
                    final_val_metrics.get("accuracy_100cp", 0)
                )
                training_history["val_accuracy_200cp"].append(
                    final_val_metrics.get("accuracy_200cp", 0)
                )
                training_history["val_median_error_centipawns"].append(
                    final_val_metrics.get("median_error_centipawns", 0)
                )
                training_history["val_p95_error_centipawns"].append(
                    final_val_metrics.get("p95_error_centipawns", 0)
                )
        ...
        if final_improved:
            save_checkpoint(
                ...
                config.num_epochs - 1,
                last_train_loss,
                final_val_metrics["loss"] if final_val_metrics else None,
                config,
                "best_model.pt",
                upload_to_hf=False,
                upload_threads=upload_threads,
        ...
        final_val_loss_for_checkpoint = (
            final_val_metrics["loss"] if final_val_metrics else last_val_loss
        )
        save_checkpoint(
            model,
            optimizer,
