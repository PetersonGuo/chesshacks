...
print(f"Sequential (for loop):     {time_seq:.4f}s  ({len(test_fens)/time_seq:.0f} pos/sec)")

# Multithreaded
start = time.time()
scores_mt = c_helpers.batch_evaluate_mt(test_fens, 0)  # 0 = auto threads
time_mt = time.time() - start
print(f"Multithreaded (all cores): {time_mt:.4f}s  ({len(test_fens)/time_mt:.0f} pos/sec)")
...
if c_helpers.is_cuda_available():
    ...
    print(f"CUDA (GPU):                {time_cuda:.4f}s  ({len(test_fens)/time_cuda:.0f} pos/sec)")
...
score1 = c_helpers.alpha_beta_optimized(
    fen, depth, c_helpers.MIN, c_helpers.MAX, True,
    c_helpers.evaluate_with_pst, tt1, 1, killer1, history1
)
...
score2 = c_helpers.alpha_beta_optimized(
    fen, depth, c_helpers.MIN, c_helpers.MAX, True,
    c_helpers.evaluate_with_pst, tt2, 0, killer2, history2
)
