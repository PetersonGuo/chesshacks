...
Run from build directory: cd build && python ../tests/performance_profile.py
"""
...
# Ensure we're in the build directory where c_helpers is located
...
def profile_search_methods():
    ...
    try:
        ...
        score_basic = c_helpers.alpha_beta_basic(
            fen, depth, c_helpers.MIN, c_helpers.MAX, True, simple_evaluate
        )
    ...
    start = time.time()
    score_par = c_helpers.alpha_beta_optimized(
        fen,
        depth,
        c_helpers.MIN,
        c_helpers.MAX,
...
def profile_evaluation_functions():
    ...
    # PST evaluation
    print("Testing evaluate_with_pst...")
    start = time.time()
    for _ in range(iterations):
        c_helpers.evaluate_with_pst(fen)
...
def profile_transposition_table():
    ...
    for depth in range(2, 7):
        ...
        start = time.time()
        c_helpers.get_best_move_uci(fen, depth, c_helpers.evaluate_with_pst, tt)
    ...
    start = time.time()
    c_helpers.get_best_move_uci(fen, 5, c_helpers.evaluate_with_pst, tt)
