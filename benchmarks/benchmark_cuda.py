...
def main():
    ...
    for depth in depths:
        ...
        def cpu_search(fen, d):
            return c_helpers.alpha_beta_optimized(
                fen, d, c_helpers.MIN, c_helpers.MAX, True,
    ...
    if cuda_available and cuda_times:
        avg_speedup = sum(cpu_times[i] / cuda_times[i] for i in range(len(cuda_times))) / len(cuda_times)
