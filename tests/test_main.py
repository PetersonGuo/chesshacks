...
def has_cuda():
    ...
    try:
        import subprocess
        result = subprocess.run(['nvidia-smi'], capture_output=True, timeout=1, check=False)
...
def search_position(fen: str, depth: int = SEARCH_DEPTH) -> int:
    ...
    if USE_CUDA:
        return c_helpers.alpha_beta_cuda(
            fen, depth, c_helpers.MIN, c_helpers.MAX, 
            True, nnue_evaluate, transposition_table
        )
    else:
        return c_helpers.alpha_beta_optimized(
            fen, depth, c_helpers.MIN, c_helpers.MAX,
            True, nnue_evaluate, transposition_table, NUM_THREADS
        )
