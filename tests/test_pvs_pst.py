...
def test_evaluate_with_pst():
    ...
    developed_fen = "r1bqkb1r/pppp1ppp/2n2n2/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4"
...
def test_pvs_performance():
    ...
    start = time.time()
    result = c_helpers.alpha_beta_optimized(
        fen, 6, c_helpers.MIN, c_helpers.MAX, True,
        c_helpers.evaluate_with_pst,
        tt, 0, killers, history
    )
    elapsed = time.time() - start
...
def test_pvs_null_window():
    ...
    start = time.time()
    result = c_helpers.alpha_beta_optimized(
        fen, 4, c_helpers.MIN, c_helpers.MAX, True,
        c_helpers.evaluate_with_pst,
        tt, 0, killers, history
    )
    elapsed = time.time() - start
